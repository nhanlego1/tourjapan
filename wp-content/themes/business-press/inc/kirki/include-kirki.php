...
if ( ! class_exists( 'Kirki' ) ) {
	...
	if ( class_exists( 'WP_Customize_Section' ) && ! class_exists( 'business_press_Installer_Control' ) ) {
		/**
		 ...
		 */
		class business_press_Installer_Control extends WP_Customize_Control {
			...
			public function render_content() {
				...
				<?php if ( ! $installed ) : ?>
					...
					<a class="install-now button-primary button" data-slug="kirki" href="<?php echo esc_url( $plugin_install_url ); ?>" aria-label="Install Kirki Toolkit now" data-name="Kirki Toolkit"><?php esc_html_e( 'Install Now','business-press' ); ?></a>

					<br/></br><!-- Added <br/> tags to fix the spacing -->

				<?php else : ?>
					<hr>
					<p><?php printf( __( 'The plugin is installed but not activated. Please <a href="%s">activate it</a>.', 'business-press' ), esc_url_raw( admin_url( 'plugins.php' ) ) ); ?></p>
				<?php endif;
			}
		}
	...
	if ( ! function_exists( 'business_press_installer_register' ) ) {
		/**
		 * Registers the section, setting & control for the kirki installer.
		 *
		 * @param object $wp_customize The main customizer object.
		 */
		function business_press_installer_register( $wp_customize ) {
			// Add the section.
			...
			$wp_customize->add_section( 'kirki_installer', array(
				'title'       => '',
				'description' => esc_attr__( 'If you want to take full advantage of the options this theme has to provide, please install the Kirki plugin.', 'business-press' ),
				'priority'    => -10,
			...
			$wp_customize->add_control( new business_press_Installer_Control( $wp_customize, 'kirki_installer', array(
				'section' => 'kirki_installer',
			) ) );

		}
		add_action( 'customize_register', 'business_press_installer_register' );
	}
}
