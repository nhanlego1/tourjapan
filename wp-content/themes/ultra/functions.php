...
define( 'SITEORIGIN_THEME_VERSION' , '1.1.6.2' );
...
function ultra_scripts() {
	...
	// Selectivizr.
	wp_enqueue_script( 'ultra-selectivizr', get_template_directory_uri() . '/js/selectivizr' . SITEORIGIN_THEME_JS_PREFIX . '.js', array(), '1.0.2' );
...
function ultra_excerpt_more( $more ) {
	...
	if ( siteorigin_setting( 'blog_archive_content' ) == 'excerpt' && siteorigin_setting( 'blog_excerpt_more' ) ) {
		...
		return '<p><span class="more-wrapper"><a class="more-link" href="' . get_permalink() . '">' . $read_more_text . '</a></span></p>';
	}
}
endif;
...
function ultra_render_slider() {
	if ( is_front_page() && ! in_array( siteorigin_setting( 'home_slider' ), array( '', 'none' ) ) ) {
		...
		if ( ( is_page() || $is_wc_shop ) && get_post_meta( $page_id, 'ultra_metaslider_slider', true ) != 'none' ) {
...
function ultra_read_more_link() {
	$read_more_text = siteorigin_setting( 'blog_read_more' ) ? esc_html( siteorigin_setting( 'blog_read_more' ) ) : esc_html__( 'Continue reading', 'ultra' );
	return '<span class="more-wrapper"><a class="more-link" href="' . get_permalink() . '">' . $read_more_text . '</a></span>';
}
