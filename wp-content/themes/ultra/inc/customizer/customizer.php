...
class SiteOrigin_Customizer_Helper {
	...
	function customize_register($wp_customize){
		...
		$wp_customize->add_panel( 'siteorigin_theme_settings', array(
			...
			'title'          => __('Theme Design', 'ultra'),
			'description'    => sprintf( __('Theme Specific Customizations For %s.', 'ultra'), $theme->get('Name') ),
		) );
	...
	function admin_enqueue( ){
		...
		wp_localize_script('siteorigin-customizer-admin', 'soCustomizeAdmin', array(
			'confirm' => __('Are you sure you want to reset your theme customizations?', 'ultra'),
			'button' => __('Reset Customizations', 'ultra'),
			'action' => wp_nonce_url(
				add_query_arg( array(
