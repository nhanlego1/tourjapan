	...
	function customize_register( $wp_customize ){
		...
		if( method_exists($wp_customize, 'add_panel') ) {
			$wp_customize->add_panel( 'page_settings', array(
				'title' => __( 'Page Template Settings', 'ultra' ),
				'description' => __( 'Change layouts for various pages on your site.', 'ultra' ),
				'priority' => 11,
			) );
		}
		...
		$types = array();
		$templates = array(
			'home' => __( 'Blog Page', 'ultra' ),
			'search' => __( 'Search Results', 'ultra' ),
			'date' => __( 'Date Archives', 'ultra' ),
			'author' => __( 'Author Archives', 'ultra' ),
			'404' => __( 'Not Found', 'ultra' ),
		);
		...
		foreach( $post_types as $post_type => $post_type_data ) {
			...
			$types[] = array(
				'group' => 'archive',
				'id' => $post_type,
				'title' => __( 'Type', 'ultra' ) . ': ' . $post_type_data->label
			);
		}
		...
		foreach( $taxonomies as $tax_slug => $taxonomy ) {
			...
			$types[] = array(
				'group' => 'taxonomy',
				'id' => $tax_slug,
				'title' => __( 'Taxonomy', 'ultra' ) . ': ' . $taxonomy->label
			);
		}
